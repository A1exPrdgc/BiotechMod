...
import net.A1exPrdgc.biotechmod.container.RefineryContainer;
...
public class RefineryScreen extends ContainerScreen<RefineryContainer>
{
	...
	protected void drawGuiContainerBackgroundLayer(MatrixStack matrixStack, float partialTicks, int x, int y)
	{
		...
		//gère la flèche
		if (this.timer >= 0 && this.nbItemInMainSlot > 1 && this.fluidAmount <= RefineryTile.CAPACITY)
		{
	...
	protected void renderHoveredTooltip(MatrixStack matrixStack, int x, int y){
		...
		if(x >= guiLeft + 122 && x < guiLeft + 122 + SqueezerScreen.TANK_SIZE_X && y >= guiTop + 14 + (SqueezerScreen.TANK_SIZE_Y - temp) && y < guiTop + 14 + TANK_SIZE_Y){
			this.renderTooltip(matrixStack, new StringTextComponent(this.fluidAmount + "/" + RefineryTile.CAPACITY), x, y);
		}
	}

	private static int sizedBar(int value)
	{
		return (SqueezerScreen.TANK_SIZE_Y * value) / RefineryTile.CAPACITY;
	}

	public static int sizedArrow(int value)
	{
		return (SqueezerScreen.ARROW_SIZE_X * value) / RefineryTile.COOKING_TIME_TICK;
	}
