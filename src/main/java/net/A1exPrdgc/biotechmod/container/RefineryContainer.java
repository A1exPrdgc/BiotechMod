...
import net.A1exPrdgc.biotechmod.tileentity.RefineryTile;
...
{
	private RefineryTile tileEntity;
	private final PlayerEntity playerEntity;
	private final IItemHandler playerInventory;

	public RefineryContainer(int windowId, World world, BlockPos pos, PlayerInventory playerInventory, PlayerEntity player)
	{
		super(ModContainers.EXTRACTOR_CONTAINER.get(), windowId);


		this.tileEntity = (RefineryTile) world.getTileEntity(pos);
		this.playerEntity = player;
		this.playerInventory = new InvWrapper(playerInventory);

		layoutPlayerInventorySlots(8, 86);

		if(this.tileEntity instanceof RefineryTile)
		{
		...
		this.tileEntity.onTankChanged();
	}

	public RefineryTile getTileEntity(){
		return tileEntity;
	}
	...
	public boolean canInteractWith(PlayerEntity playerIn){
		return isWithinUsableDistance(IWorldPosCallable.of(
				tileEntity.getWorld(), tileEntity.getPos()), playerIn, ModBlocks.REFINERY.get());
